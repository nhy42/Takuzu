...
//if (printSteps) {printf("\n");}

int recursiveSolve(SIZEDGRID * usergrid, MOVE ** moveList, int printSteps) {
    int x, y, val;
    while(giveHint(*usergrid, &x, &y, &val)) {
        if (printSteps) {printf("%d en (%d, %d) grace au calcul d'indice.\n", val, x, y);}
        *moveList = newMoveWithValues(x, y, 0, 0, *moveList);
        usergrid->grid[x][y] = val;
    }
    if (printSteps) {printf("Aucune case certaine restante\n");}
    if (isGridValid(*usergrid)) {
        if (printSteps) {printf("La grille est valide\n");}
        if (checkEnded(*usergrid)) {
            if (printSteps) {printf("La grille est resolue. Sortie.\n");}
            //printf("Go b");
            return 1;
        } else {
            int result;
            int maxRank = countEmpty(*usergrid) * 2;  // total number of guesses to do
            if (printSteps) {printf("%d Valeurs possibles pour les cases restantes\n", maxRank);}
            ...
            do {
                ...
                *moveList = newMoveWithValues(x, y, 1, 0, *moveList);
                usergrid->grid[x][y] = val;
                if (printSteps) {printf("Hypothese: %d en (%d, %d)    -> %d/%d\n", val, x, y, rank, maxRank);}
                result = recursiveSolve(usergrid, moveList, printSteps);
                rank++;
                if (result == 0) {
                    if (printSteps) {printf("Annulation %d en (%d, %d)\n", val, x, y);}
        ...
        if (printSteps) {printf("La grille n'est pas valide, retour a la dernier hypothese\n");}
        return 0;
    }
}
